...
import provinceData from '../data/data-flag-provinces-canada.json'

function Province() {
  const { id } = useParams()
  const stateInfo = provinceData.find(state => state.id === parseInt(id))
      ...
      <nav className="state__navbar">
        <ul className='state__navbar__list'>
          ...
          <li className='state__navbar__gallery-title'>
            ...
            <Link to="/gallery-can"><FontAwesomeIcon icon={faTable} className='state__navbar__title__logo'/>Back to gallery</Link>
          </li>
        </ul>
      </nav>
